...
struct AspectVGrid_Previews: PreviewProvider {
    static var previews: some View {
        AspectVGrid(items: Array<String>(repeating: "🚲", count: 10), aspectRatio: 2/3) { emoji in
            ForEach(0..<10, id: \.self) { index in
                CardView(card: EmojiMemoryGame.Card(isFaceUp: true, isMatched: false, content: emoji, id: index))
            }
        }
    }
}
